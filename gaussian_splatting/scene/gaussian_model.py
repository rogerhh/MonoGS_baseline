...
class GaussianModel:
    def __init__(self, sh_degree: int, config=None):
        ...
        self.device = "cuda:0" if config is None else config["model_params"]["data_device"]
    ...
    def create_pcd_from_image_and_depth(self, cam, rgb, depth, init=False):
        ...
        fused_point_cloud = torch.from_numpy(np.asarray(pcd.points)).float().cuda(self.device)
        fused_color = RGB2SH(torch.from_numpy(np.asarray(pcd.colors)).float().cuda(self.device))
        features = (
        ...
        dist2 = (
            torch.clamp_min(
                distCUDA2(torch.from_numpy(np.asarray(pcd.points)).float().cuda(self.device)),
                0.0000001,
            )
            * point_size
        )
